...
router
  .group(() => {
    ...
    router.get('/urls', [UrlsController, 'getAll']).as('urls')
    router.get('/urls/:url', [UrlsController, 'getOne']).as('urls.retrieve')
    router.post('/urls', [UrlsController, 'create']).as('urls.store')
    router.post('/urls/:url/increment', [UrlsController, 'increment']).as('urls.increment')
    router.delete('/urls/:url', [UrlsController, 'delete']).as('urls.destroy')
  })
  .use(middleware.auth())
  .prefix('/api/v1')
